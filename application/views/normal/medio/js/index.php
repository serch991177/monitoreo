<script>
    $(document).ready(function() {
        var table = $('#example').DataTable({
            ...
            //datos
            "ajax": {
                "url": "<?php echo site_url('normal/medio/index'); ?>",
                //"type": "POST"
            ...
            "columns": [{
...

                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        if (row.id_estado == 1) {
                            content += '<span id="span' + row.id_recurso + '" class="label success">HABILITADO</span>';
                        } else {
                            content += '<span id="span' + row.id_recurso + '" class="label alert">DESHABILITADO</span>';
                        }
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        var click = "modMedio(" + row.id_recurso + ");";
                        button += '&nbsp<button title="Modificar Medio" data-tooltip="Modificar Medio" class="list-bt palette-Defecto bg button" onclick="' + click + '" data-open="modalMedio"><i class="las la-edit la-2x"></i></button>';


                        var url = "cambiarEstado(" + row.id_recurso + ")";
                        button += '<button title="Cambiar de Estado" class="list-bt palette-Defecto bg button" onclick="' + url + '" type="button" value="' + row.id_recurso + '">';
    ...
    function nuevoMedio()
    {
        document.getElementById("titulo-modal").innerHTML = "Agregar Nuevo Medio";
        document.getElementById("btn_reg_dep").innerHTML = "Registrar Medio";
    ...
    function modMedio(identificador) {
        ...
        document.getElementById("btn_reg_med").innerHTML = "Modificar Medio";
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                document.getElementById("medio").value = response.response.data.nombre;

                var html = decodeHtml(response.response.data.detalle);
                tinymce.get("detalle").setContent(html);

                document.getElementById("id_recurso").value = response.response.data.id_recurso;
                document.getElementById("tipo").value = response.response.data.id_medio;
